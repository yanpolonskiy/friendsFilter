...
export class TheList extends Component {
    ...
    addTask() {
        ...
        this.setState({
            ...
            displayTasks: [].concat(this.state.displayTasks, {
                id,
                description,
                addedDate: new Date().toString()
            })
    ...
    deleteTask(id) {
        ...
        this.setState({
            ...
            displayTasks: tasks.filter((item, index) => { item.id !== id
            })
        })
    ...
    render() {
        let li = this.state.displayTasks.map((task, i) => <Task key={task.id} deleteTask={this.deleteTask} {...task} />)
